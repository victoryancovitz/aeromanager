...
{page==='performance' && <div style={{padding:'32px'}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24}}><div><div style={{fontFamily:'var(--font-serif)',fontSize:22,fontWeight:400}}>Performance</div><div style={{color:'var(--text3)',fontSize:12,marginTop:4}}>Dados tecnicos por aeronave</div></div><button className="primary" onClick={()=>setShowPOH(true)}>Importar POH</button></div><div style={{display:'grid',gap:16}}>{filteredAircraft.map(ac=>{const fields=[{l:'Motor',v:ac.engine_model||'n/a'},{l:'TBO Motor',v:ac.engine_tbo_hours?ac.engine_tbo_hours+'h':'n/a'},{l:'Horas',v:ac.total_flight_hours?Number(ac.total_flight_hours).toFixed(1)+'h':'n/a'},{l:'Base',v:ac.home_base||'n/a'}]; return(<div key={ac.id} className="card" style={{padding:20}}><div style={{fontFamily:'var(--font-mono)',fontWeight:700,color:'var(--blue)',fontSize:16,marginBottom:12}}>{ac.registration} <span style={{color:'var(--text3)',fontSize:12,fontFamily:'inherit',fontWeight:400}}>{ac.manufacturer} {ac.model}</span></div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))',gap:8}}>{fields.map(({l,v})=>(<div key={l} style={{background:'var(--bg2)',borderRadius:8,padding:'10px 14px'}}><div style={{fontSize:10,color:'var(--text3)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:4}}>{l}</div><div style={{fontFamily:'var(--font-mono)',color:'var(--text1)',fontWeight:600,fontSize:13}}>{v}</div></div>))}</div></div>);})}</div></div>}          </>)}
          </ErrorBoundary>
        </React.Suspense>
      </main>
