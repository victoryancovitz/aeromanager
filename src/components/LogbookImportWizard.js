...
function ConfidenceBadge({ value }) {
  ...
  return (
    <span style={{ background:color+'22', color, border:`1px solid ${color}55`, borderRadius:4, fontSize:11, padding:'1px 6px', fontWeight:700 }}>
      {pct}%
    </span>
  );
}
...
export default function LogbookImportWizard({ aircraft=[], onClose, onImported }) {
  ...
  const checkDuplicates = async (extracted, aircraftId) => {
    ...
    try {
      const { data: existing } = await supabase.from('flights')
        .select('date, departure_icao, destination_icao')
        .eq('aircraft_id', aircraftId);
      if (existing) {
        extracted.forEach((f, i) => {
          const match = existing.some(e =>
            e.date === f.date &&
            (e.departure_icao||'').toLowerCase() === (f.origin||'').toLowerCase() &&
            (e.destination_icao||'').toLowerCase() === (f.destination||'').toLowerCase()
          );
  ...
  const processImages = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        all.push(...await analyzeImage(b64, files[i].type));
      } catch (err) { console.error('Erro imagem', i, err); }
    }
    const seen = new Set();
  ...
  const handleImport = async () => {
    ...
    const toInsert = flights
      ...
      .map(f => ({
        ...
        user_id: user?.id,
        date: f.date,
        departure_icao: f.origin || null,
        destination_icao: f.destination || null,
        takeoff_utc: f.departure_time || null,
        landing_utc: f.arrival_time || null,
    ...
    try {
      ...
      // Atualiza total_flight_hours da aeronave se o diário tiver horas acumuladas
      const maxHours = flights
        .filter((_, i) => !skipDuplicates[i])
        .reduce((max, f) => Math.max(max, f.total_flight_hours || 0), 0);
      if (maxHours > 0) {
        const { data: ac } = await supabase.from('aircraft')
          .select('total_flight_hours').eq('id', selectedAircraftId).single();
        if (ac && (ac.total_flight_hours || 0) < maxHours) {
          await supabase.from('aircraft')
            .update({ total_flight_hours: maxHours })
  ...
  const S = {
    ...
    btn: (v='primary') => ({ padding:'9px 20px', borderRadius:8, fontWeight:600, fontSize:14, cursor:'pointer', border:'none', transition:'all .15s',
      ...(v==='primary'?{background:'#3b82f6',color:'#fff'}:v==='secondary'?{background:'#1e293b',color:'#94a3b8',border:'1px solid #334155'}:v==='success'?{background:'#22c55e',color:'#fff'}:{background:'#ef4444',color:'#fff'}) }),
  ...
  const acSelector = (
    <div style={{ marginBottom:16 }}>
      ...
      <select value={selectedAircraftId} onChange={e=>setSelectedAircraftId(e.target.value)}
        style={{ width:'100%', padding:'9px 12px', borderRadius:8, background:'#1e293b', border:'1px solid #334155', color:'#f1f5f9', fontSize:14 }}>
  ...
  if (step === 'upload') return (
    <div style={S.overlay} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={S.modal}>
        <div style={S.header}>
          <div>
            <p style={{ color:'#f1f5f9', fontWeight:700, fontSize:18, margin:0 }}>📖 Importar do Diário de Bordo</p>
            <p style={{ color:'#64748b', fontSize:13, marginTop:2 }}>Tire fotos das páginas e a IA extrai os voos automaticamente</p>
          </div>
          <button onClick={onClose} style={{ background:'none', border:'none', color:'#64748b', fontSize:22, cursor:'pointer' }}>✕</button>
        </div>
        <div style={S.body}>
          {acSelector}
          <div style={{ border:'2px dashed #334155', borderRadius:12, padding:'36px 24px', textAlign:'center', cursor:'pointer', background:'#0f172a' }}
          ...
          {previews.length > 0 && (
            <>
              <p style={{ color:'#64748b', fontSize:13, marginTop:16 }}>{files.length} imagem{files.length!==1?'ns':''} selecionada{files.length!==1?'s':''}</p>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(110px,1fr))', gap:10, marginTop:8 }}>
                {previews.map((url,i)=>(
                  <div key={i} style={{ position:'relative', borderRadius:8, overflow:'hidden', border:'1px solid #334155', aspectRatio:'4/3' }}>
                    <img src={url} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                    <button onClick={e=>{e.stopPropagation();removeFile(i)}} style={{ position:'absolute', top:4, right:4, background:'#ef4444', border:'none', color:'#fff', borderRadius:'50%', width:20, height:20, fontSize:11, cursor:'pointer' }}>✕</button>
                  </div>
                ))}
              </div>
            </>
        ...
        <div style={S.footer}>
          ...
          <button style={{ ...S.btn('primary'), opacity:(files.length&&selectedAircraftId)?1:0.4, cursor:(files.length&&selectedAircraftId)?'pointer':'not-allowed' }}
            disabled={!files.length||!selectedAircraftId} onClick={processImages}>
            🔍 Analisar {files.length>0?`${files.length} imagem${files.length!==1?'ns':''}`:'imagens'}
          </button>
        </div>
      </div>
    </div>
  ...
  if (step === 'processing') return (
    <div style={S.overlay}>
      <div style={{ ...S.modal, maxWidth:440, alignItems:'center', padding:48, textAlign:'center' }}>
        ...
        <div style={{ background:'#1e293b', borderRadius:8, height:6, overflow:'hidden', marginTop:12, width:'100%' }}>
          <div style={{ height:'100%', background:'#3b82f6', borderRadius:8, width:`${progress.total?(progress.current/progress.total)*100:0}%`, transition:'width .4s ease' }} />
        </div>
      </div>
    </div>
  );
      ...
      <div style={S.overlay} onClick={e=>e.target===e.currentTarget&&onClose()}>
        <div style={S.modal}>
          <div style={S.header}>
            <div>
              ...
              <p style={{ color:'#64748b', fontSize:13, marginTop:2 }}>
                {toImport.length} para importar{duplicates.size>0?` · ${duplicates.size} duplicado${duplicates.size!==1?'s':''} detectado${duplicates.size!==1?'s':''}`: ''}
              </p>
            </div>
            <button onClick={onClose} style={{ background:'none', border:'none', color:'#64748b', fontSize:22, cursor:'pointer' }}>✕</button>
          </div>
          <div style={S.body}>
            {flights.length===0 ? (
              ...
              <div style={{ overflowX:'auto' }}>
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                  <thead>
                    <tr>{['Data','Rota','Dep/Pou','Duração','H.Total','Piloto','Regra','Conf.','Ação'].map(h=><th key={h} style={S.th}>{h}</th>)}</tr>
                  </thead>
                  <tbody>
                    {flights.map((f,i)=>{
                      const isDupe = duplicates.has(i);
                      const willSkip = skipDuplicates[i];
                      const hh = Math.floor((f.flight_time_minutes||0)/60);
                      const mm = (f.flight_time_minutes||0)%60;
                      return (
                        <tr key={i} style={{ background:isDupe?(willSkip?'#1e293b88':'#f59e0b08'):'transparent', opacity:willSkip?0.45:1, transition:'all .15s' }}>
                          ...
                          <td style={{ ...S.td, minWidth:80 }}>
                            {isDupe
                              ? <button onClick={()=>setSkipDuplicates(prev=>({...prev,[i]:!prev[i]}))} style={{ ...S.btn(willSkip?'secondary':'danger'), padding:'4px 10px', fontSize:11 }}>{willSkip?'+ Incluir':'✕ Pular'}</button>
            ...
            {duplicates.size>0 && (
              <div style={{ marginTop:16, background:'#f59e0b11', border:'1px solid #f59e0b33', borderRadius:8, padding:'10px 14px', fontSize:13, color:'#fbbf24' }}>
                ⚠ {duplicates.size} voo{duplicates.size!==1?'s':''} já existe{duplicates.size!==1?'m':''} no banco. Clique em <strong>+ Incluir</strong> para importar mesmo assim.
              </div>
            )}
            {error && <p style={{ color:'#ef4444', marginTop:12, fontSize:13 }}>⚠ {error}</p>}
          </div>
          <div style={S.footer}>
            ...
            <button style={{ ...S.btn('success'), opacity:(toImport.length>0&&!importing)?1:0.4, cursor:(toImport.length>0&&!importing)?'pointer':'not-allowed' }}
              disabled={!toImport.length||importing} onClick={handleImport}>
  ...
  return (
    <div style={S.overlay}>
      <div style={{ ...S.modal, maxWidth:420, alignItems:'center', padding:48, textAlign:'center' }}>
        ...
        <p style={{ color:'#64748b', fontSize:15, marginTop:10 }}>
          <span style={{ color:'#22c55e', fontWeight:700 }}>{result?.imported}</span> voo{result?.imported!==1?'s':''} importado{result?.imported!==1?'s':''}.
          {result?.skipped>0&&<><br/><span style={{ color:'#f59e0b' }}>{result.skipped}</span> duplicado{result.skipped!==1?'s':''} ignorado{result.skipped!==1?'s':''}.</>}
          <br/><span style={{ color:'#64748b', fontSize:12, marginTop:4, display:'block' }}>Hodômetro da aeronave atualizado automaticamente.</span>
        </p>
