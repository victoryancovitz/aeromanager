...
export default function Flights({ flights=[], aircraft=[], costs=[], reload, setPage, setPreselFlight }) {
    ...
    setForm(f => {
      ...
      if (k==='takeoffUtc' || k==='landingUtc') {
        ...
        if (dep && arr) {
          ...
          if (mins <= 0) {
            setTimeError('⚠ Pouso não pode ser antes da decolagem');
            n.flightTimeMinutes = 0;
          } else {
            setTimeError('');
            n.flightTimeMinutes = mins;
          }
        }
      }
      return n;
  ...
  async function submit(e) {
    ...
    const saved = await saveFlight({
      ...form,
      fuelAddedLiters: parseFloat(form.fuelAddedLiters)||0,
    ...
    if (form.autoLaunchCost && fuelCost > 0) {
      setPreselFlight(saved);
      setPage('costs');
    } else if (!form.autoLaunchCost && window.confirm('Voo salvo! Deseja lançar os custos agora?')) {
      setPreselFlight(saved);
      setPage('costs');
    }
  }
        ...
        <form onSubmit={submit}>
          <div className="card" style={{ padding:'16px 20px', marginBottom:14 }}>
            ...
            <div className="g3" style={{ marginBottom: timeError ? 4 : 14 }}>
              ...
              <div>
                <label>Pouso (UTC)</label>
                <input type="time" value={form.landingUtc} onChange={e=>set('landingUtc',e.target.value)}
                  style={{ borderColor: timeError ? '#ef4444' : undefined }} />
              </div>
              <div><label>Tempo de voo</label>
                <input readOnly value={form.flightTimeMinutes>0 ? `${h}h ${m.toString().padStart(2,'0')}min` : '—'}
                  style={{ color:'#4a9eff', fontFamily:"'JetBrains Mono',monospace", fontWeight:600 }} />
              </div>
            </div>
            ...
            <div className="g4" style={{ marginBottom:0 }}>
              <div><label>Condição</label>
                <select value={form.flightConditions} onChange={e=>set('flightConditions',e.target.value)}>
                  <option value="vfr">VMC/VFR</option><option value="ifr">IMC/IFR</option><option value="mixed">Misto</option>
                </select>
              </div>
              <div><label>Finalidade</label>
                <select value={form.purpose} onChange={e=>set('purpose',e.target.value)}>
                  <option value="leisure">Lazer</option><option value="training">Instrução</option>
                  <option value="transport">Transporte</option><option value="professional">Profissional</option>
                </select>
              </div>
          ...
          <div className="card" style={{ padding:'16px 20px', marginBottom:14 }}>
            ...
            <div className="g3">
              ...
              <div><label>Custo combustível</label>
                <input readOnly value={fuelCost>0 ? `R$ ${fuelCost.toFixed(2).replace('.',',')}` : '—'}
                  style={{ color:'#f5a623', fontFamily:"'JetBrains Mono',monospace", fontWeight:600 }} />
              </div>
            </div>
            {fuelCost > 0 && (
              <label style={{ display:'flex', alignItems:'center', gap:8, marginTop:10, fontSize:13, color:'var(--text2)', cursor:'pointer' }}>
                <input type="checkbox" checked={form.autoLaunchCost} onChange={e=>set('autoLaunchCost', e.target.checked)} />
                Lançar combustível como custo automaticamente ao salvar
              </label>
            )}
          </div>
          ...
          <div className="card" style={{ padding:'16px 20px', marginBottom:20 }}>
            ...
            <textarea placeholder="Condições meteorológicas, observações, passageiros, intercorrências..."
              value={form.logbookNotes} onChange={e=>set('logbookNotes',e.target.value)} />
          </div>
    ...
    <div style={{ padding:24 }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button
            ...
            title="Importar voos de foto do diário de bordo"
            style={{
              ...
              cursor:'pointer', fontSize:13, fontWeight:600, whiteSpace:'nowrap', transition:'all .15s'
            }}
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
            ...
            <tbody>
              {filtered.map(f=>{
                ...
                return (
                  <tr key={f.id} className={isSel?'row-selected':''} onClick={()=>startEdit(f)}
                    style={{ borderBottom:'1px solid var(--border)', cursor:'pointer' }}>
                    <td style={{ padding:'9px 14px', width:36 }} onClick={e=>e.stopPropagation()}>
                      <input type="checkbox" checked={isSel} onChange={()=>ms.toggle(f.id)} style={{ width:14, height:14 }} />
                    </td>
                    <td style={{ padding:'9px 12px', whiteSpace:'nowrap', fontFamily:'var(--font-mono)', fontSize:12 }}>
                      {new Date(f.date+'T12:00:00').toLocaleDateString('pt-BR')}
                    </td>
                    <td style={{ padding:'9px 12px' }}>
                      <span className={`tag tag-${ac?.type==='single_engine'?'mono':ac?.type==='multi_engine'?'bi':'exp'}`}>{ac?.registration||'?'}</span>
                    </td>
                    ...
                    <td style={{ padding:'9px 12px' }}>
                      <span className={`tag tag-${f.flightConditions==='ifr'?'ifr':'vfr'}`}>{(f.flightConditions||'vfr').toUpperCase()}</span>
                    </td>
                    <td style={{ padding:'9px 12px', fontFamily:'var(--font-mono)', color:'var(--blue)', fontWeight:500 }}>
                      {fc>0?`R$${fc.toLocaleString('pt-BR')}`:'—'}
                    </td>
                    <td style={{ padding:'9px 12px', color:'var(--text3)', maxWidth:160, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                      {f.logbookNotes||'—'}
                    </td>
      ...
      {showLogbookImport && (
        <LogbookImportWizard
          ...
          onImported={() => { setShowLogbookImport(false); reload(); }}
        />
      )}
    </div>
  );
